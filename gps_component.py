...
HTML5 Geolocation butonu → navigator.geolocation.getCurrentPosition()
→ URL query'ye yazar + window.location.href ile sayfayı yeniler
(Streamlit rerun tetikler) → get_gps_from_query() okur.
"""
  ...
  btn.addEventListener("click", function() {{
    ...
    navigator.geolocation.getCurrentPosition(
      function(pos) {{
        ...
        btn.disabled = false;
        // URL'e yaz + sayfayı yenile (Streamlit rerun tetiklemek için)
        const url = new URL(window.location);
        url.searchParams.set("gps_lat", lat);
        url.searchParams.set("gps_lon", lon);
        url.searchParams.set("gps_acc", acc);
        // 800ms göster ki mesaj görünsün, sonra reload
        setTimeout(function() {{
...
def gps_button(label: str = "📍 Konumumu Al", key: str = "default"):
    """HTML5 Geolocation butonu. Tıklayınca URL'e konum yazar, sayfa yenilenir,
    Streamlit tekrar açılır, get_gps_from_query() koordinatı okur."""
...
def clear_gps_query():
    """URL'den GPS query parametrelerini temizle (reload olmaz, sadece state)."""
    try:
