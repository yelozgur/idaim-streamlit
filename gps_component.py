...
Pattern:
  - Buton → navigator.geolocation.getCurrentPosition()
  - URL'e ?gps_lat=...&gps_lon=...&gps_acc=... ekle
  - window.location.href ile sayfayı yenile
  - Streamlit query_params'tan oku → session_state'e yaz
  - GPS okunduktan sonra clear_gps_query() ile URL temizlenir

Avantaj: minimum JS, tek adımda konum al → sayfayı yenile → state hazır.
"""
...
function getLocation() {{
  ...
  navigator.geolocation.getCurrentPosition(
    function(p) {{
      ...
      setTimeout(function() {{ window.location.href = url.toString(); }}, 600);
    }},
    function(e) {{
      btn.disabled = false;
