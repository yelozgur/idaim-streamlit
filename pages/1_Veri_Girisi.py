...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    # ============ 1) GPS KONTROLÜ (query param'dan, reload sonrası) ============
    ...
    if gps_coords:
        ...
        # Manuel input'ları da güncelle (ki kullanıcı değerleri görsün)
        st.session_state[f"{key_prefix}_lat"] = float(gps_lat)
        st.session_state[f"{key_prefix}_lon"] = float(gps_lon)
        st.success(f"✅ GPS konum alındı: {gps_lat:.5f}, {gps_lon:.5f}")
        clear_gps_query()
        st.rerun()

    # ============ 2) HARİTA (Folium + tıklanabilir) ============
    ...
    # Folium harita — zoom 14 (seçim varsa) veya 9 (yoksa)
    zoom = 14 if has_selection else 9
    m = folium.Map(location=[prev_lat, prev_lon], zoom_start=zoom, tiles="OpenStreetMap")

    if has_selection:
        # Seçili crosshair (kırmızı pin)
        folium.Marker(
            [prev_lat, prev_lon],
            icon=folium.DivIcon(
                html='<div style="font-size:28px;color:red;text-shadow:0 0 3px white;">📍</div>',
                icon_size=(40, 40), icon_anchor=(20, 20),
            ),
        ).add_to(m)
        folium.Circle([prev_lat, prev_lon], radius=500,
                      color="red", fill=True, fill_opacity=0.1, weight=2).add_to(m)

    # key'e koordinat ekle → konum değişince Folium yeniden oluşur (yeniden render)
    map_key = f"{key_prefix}_folium_{prev_lat:.4f}_{prev_lon:.4f}"
    map_data = st_folium(
        m, height=320, returned_objects=["last_clicked"],
        key=map_key,
    )
    # Tıklama event
    if map_data and map_data.get("last_clicked"):
        ...
        clk_lat = clk.get("lat")
        clk_lon = clk.get("lng")
        if clk_lat and clk_lon and (abs(clk_lat - prev_lat) > 0.0001 or abs(clk_lon - prev_lon) > 0.0001):
            st.session_state[f"{key_prefix}_selected_lat"] = clk_lat
            st.session_state[f"{key_prefix}_selected_lon"] = clk_lon
            st.session_state[f"{key_prefix}_lat"] = float(clk_lat)
            st.session_state[f"{key_prefix}_lon"] = float(clk_lon)
            st.rerun()
    ...
    with gps_col:
        # GPS butonu — form GET submit ile ?gps_lat=... ekler, Streamlit otomatik rerun
        gps_button(label="📡 GPS Al", key=f"{key_prefix}_gps")
        ...
        try:
            ...
            if cell_id is not None:
                ...
                with st.expander("🗺️ Seçili Hücre Detayı (trap/lab)", expanded=False):
                    ...
                    st_folium(m2, height=300, returned_objects=[], key=f"{key_prefix}_detail_map")
