...
import utils
from gps_component import gps_button, get_gps_from_postmessage, clear_gps_payload
...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    with gps_col:
        # GPS butonu (CSP-safe, postMessage)
        gps_button(label="📡 GPS Al", key=f"{key_prefix}_gps_btn", height=120)
        st.caption("👆 Telefondaysan konum izni ver")
        # Otomatik kontrol: postMessage'dan GPS geldi mi?
        gps_payload = get_gps_from_postmessage()
        if gps_payload:
            gps_lat, gps_lon, gps_acc = gps_payload
            # Aynı değer mi kontrol et (sonsuz loop önleme)
            existing = st.session_state.get(f"{key_prefix}_selected_lat")
            if existing != gps_lat or st.session_state.get(f"{key_prefix}_selected_lon") != gps_lon:
            ...
            st.caption("📍 Henüz GPS alınmadı. Butona bas veya Manuel gir.")
