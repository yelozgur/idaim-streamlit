...
import utils
from gps_component import gps_button_v4 as gps_button, get_gps_from_query, clear_gps_query
...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    st.markdown("**📍 Konum Belirle** (Manuel veya GPS)")

    # GPS butonu (varsa çalışır, yoksa skip)
    ...
    with gps_col_top:
        if st.button("📡 GPS'ten Al", key=f"{key_prefix}_gps_get", use_container_width=True, type="secondary"):
            ...
            if result:
                ...
                st.warning("⚠️ GPS alınamadı. Tarayıcı izin verdi mi? Manuel girebilirsin.")
