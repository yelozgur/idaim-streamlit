...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    # ============ 1) GPS KONTROLÜ (URL query param'dan, reload sonrası) ============
    gps_coords = get_gps_from_query()
    if gps_coords:
        gps_lat, gps_lon, gps_acc = gps_coords
        # Aynı değer mi kontrol et (sonsuz loop önleme)
        existing_lat = st.session_state.get(f"{key_prefix}_selected_lat")
        existing_lon = st.session_state.get(f"{key_prefix}_selected_lon")
        if existing_lat != gps_lat or existing_lon != gps_lon:
    ...
    with gps_col:
        # GPS butonu (v4: query parameter + location.replace, sandboxing-safe)
        gps_button(label="📡 GPS Al", key=f"{key_prefix}_gps_btn", height=120)
        st.caption("👆 Telefondaysan konum izni ver")
        st.caption("📍 Sayfa yenilenince koordinat yansır.")
