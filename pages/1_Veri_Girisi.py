...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    # CartoDB Voyager tile (ücretsiz, OSM datası, daha güzel görünüm, sandboxing-safe)
    m = folium.Map(
        location=[prev_lat, prev_lon], zoom_start=zoom,
    ...
    with gps_col:
        ...
        if st.button("📍 Konumumu Al", key=f"{key_prefix}_gps_trigger", use_container_width=True, type="primary"):
            ...
            if result:
                ...
                render_gps_fallback_button(key=f"{key_prefix}_gps_fb")
