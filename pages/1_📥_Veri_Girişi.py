...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    """Konum seçici: GPS / Harita tıkla / Manuel — 3'ü aynı hedefe.

    Akış:
    1. GPS butonu (HTML5) → koordinat query param ile gelir
    2. Folium harita (boş, sadece OSM base + crosshair + tıklama alıcı)
    3. Manuel Lat/Lon input
    ...
    # ============ 2) HARITA (Folium — ücretsiz OSM, çalışıyor) ============
    ...
    # Folium harita (sadece base + crosshair, marker loop YOK)
    ...
    if has_selection:
        ...
        folium.Marker(
            [prev_lat, prev_lon],
            icon=folium.DivIcon(
                html=f'<div style="font-size:28px;color:red;text-shadow:0 0 3px white;">📍</div>',
                icon_size=(40, 40),
                icon_anchor=(20, 20),
            ),
        ).add_to(m)
        # Trafik ışığı gibi circle — kullanıcıya "şu an buradasın" hissi
        folium.Circle(
            [prev_lat, prev_lon],
            radius=500,  # 500m
            color="red", fill=True, fill_opacity=0.1, weight=2,
        ).add_to(m)

    # Tıklama event'i st_folium üzerinden
    map_data = st_folium(
        m, height=350, returned_objects=["last_clicked"],
    ...
    with man_col:
        ...
        # Manuel değişiklik → crosshair güncelle
        ...
        try:
            ...
            if cell_id is not None:
                ...
                with st.expander("🗺️ Seçili Hücre Detayı (trap/lab)", expanded=False):
                    ...
                    for _, tr in nearby_traps.iterrows():
                        folium.CircleMarker([self_cell["lat"], self_cell["lon"]],
                            radius=8, color="orange", fill=True, fill_opacity=0.6,
                            popup=f"Trap: {tr['trap_id']}",
                        ).add_to(m2)
                    for _, lb in nearby_labs.iterrows():
                        folium.CircleMarker([self_cell["lat"], self_cell["lon"]],
                            radius=10, color="purple", fill=True, fill_opacity=0.6,
                            popup=f"Lab: {lb.get('species', '?')}",
                        ).add_to(m2)
