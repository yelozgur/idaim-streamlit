...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    # ============ YÖNTEM 2: HARİTADAN TIKLA ============
        ...
        if len(cells) > 0:
            # Density heatmap — 37K hücre hızlı, zoom-responsive
            # Renk: culex_proba (yoksa 0.5 default gri ton)
            ...
            fig.add_trace(go.Densitymapbox(
                ...
                zmin=0, zmax=1,
                showscale=True,
                colorbar=dict(title="ML proba", x=1.02),
                name="Yoğunluk",
            ))
            ...
            if "selected_lat" in st.session_state and "selected_lon" in st.session_state:
                ...
                fig.add_trace(go.Scattermap(
                    ...
                    mode="markers",
                    marker=dict(size=20, color="blue", symbol="cross"),
                    name="Seçili konum",
                ))
            fig.update_layout(
            ...
            st.plotly_chart(
                ...
                key=f"{key_prefix}_density_map",
            )

            # Manuel konum seçimi (density haritası interaktif tıklama yok)
            st.caption("📍 Harita üzerinde konum seçmek için aşağıdan Lat/Lon girin")
            col1, col2 = st.columns(2)
            with col1:
                lat_input = st.number_input(
                    "Lat (tıkla veya gir)", value=34.9, format="%.5f",
                    key=f"{key_prefix}_map_lat",
                )
            with col2:
                lon_input = st.number_input(
                    "Lon (tıkla veya gir)", value=33.2, format="%.5f",
                    key=f"{key_prefix}_map_lon",
                )
            if lat_input and lon_input:
                # Session state'e kaydet (crosshair için)
