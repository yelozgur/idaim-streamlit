...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    if method == "📡 GPS":
        ...
        if len(cells) > 0:
            ...
            # MarkerCluster ile 37K hücre performanslı render
            ...
            # Toplu marker ekleme (iloc ile, itertuples değil)
            for idx in range(len(cells_view)):
                ...
                folium.CircleMarker(
                    ...
                    radius=4,
                    popup=(
                        f"cell_id={row['cv_cell_id_str']}<br>"
                        f"lat={row['lat']}<br>"
                        f"lon={row['lon']}<br>"
                        f"proba={row.get('culex_proba', 'N/A')}"
                    ),
                    color=row["cv_color"],
                    fill=True,
                    fill_opacity=0.4,
