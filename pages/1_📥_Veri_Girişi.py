...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    zoom = 13 if has_selection else 9
    ...
    with gps_col:
        # GPS butonu — JS Manuel input'a yazıp change event tetikler (Streamlit rerun)
        lat_key = f"{key_prefix}_lat"
        lon_key = f"{key_prefix}_lon"
        gps_button(
            label="📡 GPS Al",
            key=f"{key_prefix}_gps",
            lat_input_key=lat_key,
            lon_input_key=lon_key,
        )
