...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    # ============ YÖNTEM 2: HARİTADAN TIKLA ============
        ...
        if len(cells) > 0:
            # Density heatmap + tıklanabilir scatter layer
            # 37K hücre yoğunluk haritası + tıklayınca scatter event
            ...
            fig.add_trace(go.Densitymapbox(
                ...
                radius=8, colorscale="Reds", zmin=0, zmax=1,
                showscale=True, colorbar=dict(title="ML proba", x=1.02),
                name="Yoğunluk", hoverinfo="skip",
            ))
            # Tıklanabilir invisible layer (tıklama alıcı)
            # 5x5 grid spread tüm haritaya, kullanıcı herhangi bir yere tıklayabilir
            grid_lats = np.linspace(34.5, 35.7, 25)
            grid_lons = np.linspace(32.3, 34.6, 25)
            grid_lat_mesh, grid_lon_mesh = np.meshgrid(grid_lats, grid_lons)
            fig.add_trace(go.Scattermap(
                lat=grid_lat_mesh.flatten(),
                lon=grid_lon_mesh.flatten(),
                mode="markers",
                marker=dict(size=14, color="rgba(0,0,0,0)", opacity=0.01),  # görünmez ama tıklanabilir
                name="tıklama_alıcı", hoverinfo="skip",
                showlegend=False,
            ))
            ...
            fig.add_trace(go.Scattermap(
                ...
                mode="markers",
                marker=dict(size=22, color="blue", symbol="cross",
            ...
            event = st.plotly_chart(
                ...
                on_select="rerun", key=f"{key_prefix}_density_map",
            )
            # Tıklama kontrolü
            if event and event.selection and len(event.selection.points) > 0:
                points = event.selection.points
                # Son noktayı al (en son tıklama)
                for pt in points:
                    pt_lat = pt.get("lat")
                    pt_lon = pt.get("lon")
            ...
            # Manuel override
            st.caption("👆 Haritada tıkla veya aşağıdan elle gir")
            col1, col2 = st.columns(2)
            with col1:
                lat_input = st.number_input(
                    "Lat", value=prev_lat, format="%.5f",
                    key=f"{key_prefix}_map_lat",
                )
            with col2:
                lon_input = st.number_input(
                    "Lon", value=prev_lon, format="%.5f",
                    key=f"{key_prefix}_map_lon",
                )
            # Eğer input değiştiyse crosshair güncelle
