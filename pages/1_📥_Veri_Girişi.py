...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    """Konum seçici: 3 yöntem aynı harita üzerinde.

    Akış:
    1. Harita HER ZAMAN görünür (density heatmap + tıklanabilir)
    2. Kullanıcı 3 yöntemden biriyle konum seçer:
       - GPS butonu → haritada crosshair + zoom
       - Haritaya tıkla → crosshair
       - Manuel Lat/Lon → crosshair
    3. Seçim → en yakın hücre otomatik bulunur
    ...
    if len(cells) == 0:
        st.warning("Hücre verisi yok")
        return None

    cells_view = cells.copy()
    cells_view["cv_proba"] = pd.to_numeric(cells_view["culex_proba"], errors="coerce").fillna(0.5)

    # Önceki seçim (default Cyprus merkezi)
    ...
    # Harita zoom seviyesi (seçim varsa yakın, yoksa Cyprus genel)
    map_zoom = 12 if has_selection else 9
    map_center = (prev_lat, prev_lon) if has_selection else (34.9, 33.2)

    fig = go.Figure()
    # Density (görsel)
    fig.add_trace(go.Densitymapbox(
        lat=cells_view["lat"], lon=cells_view["lon"],
        z=cells_view["cv_proba"],
        radius=10, colorscale="Reds", zmin=0, zmax=1,
        showscale=True, colorbar=dict(title="ML proba", x=1.02, len=0.7),
        name="Yoğunluk", hoverinfo="skip",
    ))
    # Tıklanabilir invisible layer
    grid_lats = np.linspace(34.5, 35.7, 30)
    grid_lons = np.linspace(32.3, 34.6, 30)
    ...
    fig.add_trace(go.Scattermap(
        ...
        name="tıklama", hoverinfo="skip", showlegend=False,
    ))
    # Seçili crosshair (varsa)
    if has_selection:
        fig.add_trace(go.Scattermap(
            lat=[prev_lat], lon=[prev_lon],
            mode="markers",
            marker=dict(size=22, color="red", symbol="cross"),
            name="Seçili", hoverinfo="text",
            text=f"📍 {prev_lat:.5f}, {prev_lon:.5f}",
        ))
    fig.update_layout(
        map=dict(style="open-street-map", center=dict(lat=map_center[0], lon=map_center[1]), zoom=map_zoom),
        height=350, margin=dict(t=0, b=0, l=0, r=0), showlegend=False,
    )
    # Tıklama event
    event = st.plotly_chart(
        fig, use_container_width=True,
        config={"displayModeBar": False, "displaylogo": False},
        on_select="rerun", key=f"{key_prefix}_map",
    )
    if event and event.selection and len(event.selection.points) > 0:
        for pt in event.selection.points:
            ...
            if pt_lat and pt_lon:
                st.session_state[f"{key_prefix}_selected_lat"] = pt_lat
                st.session_state[f"{key_prefix}_selected_lon"] = pt_lon
                st.rerun()

    # ============ 3) GPS + MANUEL BUTONLARI (haritanın altında) ============
    st.markdown("##### 📍 Konum Belirle")
    gps_col, man_col = st.columns([1, 2])

    with gps_col:
        # HTML5 Geolocation butonu
        gps_button(label="📡 GPS", key=f"{key_prefix}_gps")
        st.caption("Konum izni verin")
    ...
    with man_col:
        ...
        # Manuel değişiklik → harita güncelle
