...
import folium  # noqa: F401  # legacy import, may be used by other functions
# st_folium kaldırıldı — refresh sorunu nedeniyle
...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    # ============ 1) GPS + MANUEL (inline, basit) ============
    # 4 tur sandboxing denedik, en sonunda:
    # - st.map refresh olmuyor
    # - postMessage unregistered oluyor
    # - location.replace sandboxing blokluyor
    # KARAR: haritayı SONRA ekleriz, şimdilik sadece Manuel + inline GPS
    st.markdown("**📍 Konum Belirle** (Manuel gir, veya GPS'ten al)")

    # GPS butonu (CSP-safe placeholder, Manuel önerilir)
    gps_col_top, _ = st.columns([1, 3])
    with gps_col_top:
        if st.button("📡 GPS'ten Al", key=f"{key_prefix}_gps_get", use_container_width=True, type="secondary"):
            result = get_gps()
            if result:
                ...
                st.success(f"✅ GPS: {gps_lat:.5f}, {gps_lon:.5f} (±{gps_acc}m) — aşağıdaki input'a yansıdı")
            else:
                st.info("💡 GPS şu an kullanılamıyor (CSP kısıtı). Manuel koordinat girin.")

    # Manuel input (satır satır, temiz)
    col_a, col_b = st.columns(2)
    with col_a:
        lat_input = st.number_input(
            "Latitude (Enlem)", value=prev_lat, format="%.5f",
    ...
    with col_b:
        lon_input = st.number_input(
            "Longitude (Boylam)", value=prev_lon, format="%.5f",
            key=f"{key_prefix}_lon",
            on_change=_on_man_coord_change,
            args=(key_prefix, "lon"),
        )
    # Manuel değişiklik → state güncelle
    ...
    if has_selection or (lat_input and lon_input):
        ...
        try:
            ...
            if cell_id is not None:
                ...
                # ============ 5) TRAP/LAB LİSTESİ (harita yok, sadece liste) ============
                with st.expander("🗺️ Seçili Hücre Detayı (trap/lab)", expanded=False):
                    ...
                    if len(cell_matches) == 0:
                        ...
                        st.write(f"**Hücre #{cell_id}** — Lat: {self_cell['lat']:.5f}, Lon: {self_cell['lon']:.5f}")
                        ...
                        n_traps = len(nearby_traps)
                        st.write(f"🪤 **Trap'ler:** {n_traps} aktif trap")
                        if n_traps > 0 and "trap_id" in nearby_traps.columns:
                            for tr_id in nearby_traps["trap_id"].head(5):
                                st.caption(f"  - {tr_id}")
                            if n_traps > 5:
                                st.caption(f"  ... +{n_traps - 5} daha")
                        ...
                        n_labs = len(nearby_labs)
                        st.write(f"🧪 **Lab sonuçları:** {n_labs} kayıt")
                        if n_labs > 0 and "species" in nearby_labs.columns:
                            species_count = nearby_labs["species"].value_counts().head(3)
                            for sp, cnt in species_count.items():
                                st.caption(f"  - {sp}: {cnt}")
                        st.caption("💡 Harita gösterimi sonra eklenecek.")
...
def tab_lab_result():
    ...
    # ============ TRAP KAPAT (saha akışı: check → kapat → lab) ============
    current_state = str(init_row.iloc[0].get("state", "")).strip().lower()
    if current_state != "closed":
        st.info("🪤 **Saha adımı:** Lab'a göndermeden önce trap'i kapat.")
        with st.form("close_trap_form", clear_on_submit=True):
            st.subheader(f"🪤 Trap Kapat — {selected_trap}")
            close_col1, close_col2 = st.columns(2)
            with close_col1:
                close_date = st.date_input("Kapatma Tarihi *", value=datetime.now().date(), key="close_date")
            with close_col2:
                close_method = st.selectbox(
                    "Kapatma Yöntemi",
                    ["Planlı", "Erken (kullanıcı bulunamadı)", "Hasarlı", "Diğer"],
                    key="close_method",
                )
            close_note = st.text_input("Not (opsiyonel)", key="close_note")
            close_submitted = st.form_submit_button("🪤 Trap'i Kapat", type="primary", use_container_width=True)
            if close_submitted:
                # state'i güncelle (sampling_initiation tablosunda satır bul, state + closed_at yaz)
                ws = sheets_client.get_worksheet("sampling_initiation")
                headers = ws.row_values(1)
                all_rows = ws.get_all_values()
                # trap_id satırını bul
                trap_id_idx = headers.index("trap_id") + 1  # 1-based
                target_row = None
                for i, row in enumerate(all_rows[1:], start=2):  # 1-based, header'ı atla
                    if len(row) >= trap_id_idx and row[trap_id_idx - 1] == selected_trap:
                        target_row = i
                        break
                if target_row is None:
                    st.error("Trap satırı bulunamadı")
                else:
                    # state kolonu
                    if "state" in headers:
                        state_col = headers.index("state") + 1
                        ws.update_cell(target_row, state_col, "closed")
                    # closed_at kolonu (yoksa ekle)
                    if "closed_at" in headers:
                        closed_at_col = headers.index("closed_at") + 1
                        ws.update_cell(target_row, closed_at_col, close_date.isoformat())
                    # closed_method
                    if "closed_method" in headers:
                        method_col = headers.index("closed_method") + 1
                        ws.update_cell(target_row, method_col, close_method)
                    # closed_note
                    if "closed_note" in headers:
                        note_col = headers.index("closed_note") + 1
                        ws.update_cell(target_row, note_col, close_note)
                    # Cache temizle
                    sheets_client._read_sheet_cached.clear()
                    st.success(f"✅ Trap {selected_trap} kapatıldı. Şimdi Lab girişi yapabilirsin.")
                    st.rerun()
    else:
        st.success("🪤 Trap zaten kapalı. Lab girişi yapabilirsin.")
