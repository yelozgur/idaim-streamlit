...
def render_location_picker(label: str, key_prefix: str) -> tuple[float, float, int] | None:
    ...
    # ============ 2) HARİTA (st.map — Folium sandbox sorunları YOK) ============
    ...
    # Basit harita — seçili pin ortada
    ...
    st.map(map_df, zoom=11, size=200, color="#ff0000")

    st.caption(f"📍 Mevcut: {prev_lat:.5f}, {prev_lon:.5f}" + (" (seçili)" if has_selection else " (varsayılan Cyprus merkezi)"))
