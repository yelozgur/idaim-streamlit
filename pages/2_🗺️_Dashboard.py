...
cells_view["_proba"] = pd.to_numeric(cells_view[proba_col], errors="coerce")
cells_view["_color"] = cells_view["_proba"].apply(lambda p: proba_to_color(p, threshold=0.10))
cells_view["_cell_id_str"] = cells_view["cell_id"].apply(safe_cell_id)
cells_view["_tooltip"] = cells_view.apply(
    lambda r: f"#{r['_cell_id_str']} | {r.get('district', '?')} | {fmt_proba(r['_proba'])}",
    axis=1
)
...
# Toplu marker ekleme (iterrows yerine itertuples - 5x hızlı)
for row in cells_view.itertuples(index=False):
    proba = row._proba
    color = row._color
    popup_html = f"""
    <b>Hücre #{row._cell_id_str}</b><br>
    District: {row.district}<br>
    Lat/Lon: {row.lat:.4f}, {row.lon:.4f}<br>
    <hr>
    <b>Culex proba:</b> {fmt_proba(row.culex_proba)}<br>
    <b>Aedes proba:</b> {fmt_proba(row.aedes_proba)}<br>
    <b>Confidence:</b> {row.confidence_tier}<br>
    <b>Son güncelleme:</b> {row.last_updated}
    """
    folium.CircleMarker(
...
# Bounds'u session_state'e kaydet (sonraki render için)
if map_data and map_data.get("bounds"):
    st.session_state["last_map_bounds"] = map_data["bounds"]
