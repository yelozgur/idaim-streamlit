...
# Hücreler (CircleMarker + MarkerCluster performans)
from folium.plugins import MarkerCluster

# Proba kolonunu vectorize parse
cells_view = cells.copy()
cells_view["cv_proba"] = pd.to_numeric(cells_view[proba_col], errors="coerce")
cells_view["cv_color"] = cells_view["cv_proba"].apply(lambda p: proba_to_color(p, threshold=0.10))
cells_view["cv_cell_id_str"] = cells_view["cell_id"].apply(safe_cell_id)
cells_view["cv_tooltip"] = cells_view.apply(
    lambda r: f"#{r['cv_cell_id_str']} | {r.get('district', '?')} | {fmt_proba(r['cv_proba'])}",
    axis=1
)

# Viewport filter (önceki render'dan gelen bounds)
# İlk render'da bounds None, tüm hücreler gösterilir
# Sonraki render'larda sadece viewport'taki hücreler
last_bounds = st.session_state.get("last_map_bounds")
if last_bounds:
    # st_folium bounds formatı değişken olabilir — güvenli parse
    try:
        if isinstance(last_bounds, list) and len(last_bounds) >= 2:
            # Eski format: [[south, west], [north, east]]
            if isinstance(last_bounds[0], (list, tuple)):
                south, west = last_bounds[0]
                north, east = last_bounds[1]
            # Yeni format: [{'_southWest': {...}, '_northEast': {...}}] (Leaflet)
            elif isinstance(last_bounds[0], dict):
                sw = last_bounds[0]
                ne = last_bounds[1]
                south = sw.get("lat", sw.get("_southWest", {}).get("lat"))
                west = sw.get("lng", sw.get("_southWest", {}).get("lng"))
                north = ne.get("lat", ne.get("_northEast", {}).get("lat"))
                east = ne.get("lng", ne.get("_northEast", {}).get("lng"))
            else:
                raise ValueError(f"Unknown bounds format: {type(last_bounds[0])}")
        else:
            raise ValueError(f"Invalid bounds: {last_bounds}")

        cells_view_filtered = cells_view[
            (cells_view["lat"] >= south) & (cells_view["lat"] <= north) &
            (cells_view["lon"] >= west) & (cells_view["lon"] <= east)
        ]
        if len(cells_view_filtered) > 0:
            cells_view = cells_view_filtered
            st.caption(f"📍 Viewport: {len(cells_view):,} hücre gösteriliyor (toplam {len(cells):,})")
    except Exception as e:
        # Bounds parse hatası — tüm hücreleri göster
        st.session_state.pop("last_map_bounds", None)

# MarkerCluster (performans için)
cells_layer = folium.FeatureGroup(name="Hücreler", show=True)
marker_cluster = MarkerCluster(
    name="Hücre Cluster",
    options={
        "maxClusterRadius": 50,  # 50px yarıçapta küme
        "disableClusteringAtZoom": 13,  # Zoom 13'te marker'lar açılır
        "spiderfyOnMaxZoom": True,
        "showCoverageOnHover": False,
    }
).add_to(cells_layer)

# Toplu marker ekleme (itertuples kullanmıyoruz — Pandas underscore'lu isimleri skip eder)
# Index bazlı erişim veya iloc kullan, daha güvenli
for idx in range(len(cells_view)):
    row = cells_view.iloc[idx]
    popup_html = f"""
    <b>Hücre #{row['cv_cell_id_str']}</b><br>
    ...
    folium.CircleMarker(
        ...
        tooltip=row["cv_tooltip"],
        color=row["cv_color"],
        fill=True,
        fill_color=row["cv_color"],
        fill_opacity=0.6,
        weight=1,
...
# Haritayı göster (bounds döndür, sonraki render'da viewport filter uygula)
map_data = st_folium(
    m,
    height=550,
...
# Bounds'u session_state'e kaydet (sonraki render için, güvenli parse)
if map_data and map_data.get("bounds"):
    try:
        bounds = map_data["bounds"]
        # Liste formatını doğrula: [[south, west], [north, east]] veya list of dicts
        if isinstance(bounds, list) and len(bounds) >= 2:
            if isinstance(bounds[0], (list, tuple)) and len(bounds[0]) >= 2:
                st.session_state["last_map_bounds"] = bounds
            elif isinstance(bounds[0], dict):
                st.session_state["last_map_bounds"] = bounds
    except Exception:
        pass

# Viewport-based filter (performans optimizasyonu)
# İlk render'da bounds None olur, tüm hücreleri göster (zoom 9, tüm ada)
# Sonra viewport değişince sadece o alandaki hücreleri render et
if map_data and map_data.get("bounds"):
    bounds = map_data["bounds"]
    ...
    # Sadece viewport'taki hücreleri render et (bir sonraki render için)
    # Not: bu sadece bilgi amaçlı, gerçek performans kazancı için render loop'ta filtreleme lazım
    visible_count = ((cells_view["lat"] >= south) & (cells_view["lat"] <= north) &
                     (cells_view["lon"] >= west) & (cells_view["lon"] <= east)).sum()

    # Çok fazlaysa (>10K), kullanıcıya viewport filter öner
    if len(cells_view) > 10000 and visible_count == len(cells_view):
        st.info("💡 Haritada 10K+ hücre var. Filtreleri (district, confidence) kullanın veya haritayı yakınlaştırın.")


# Harita render sonrası marker ekleme (viewport filter uygulandı)
# NOT: Performans için bu noktada bounds'a göre filtre uygulanabilir
